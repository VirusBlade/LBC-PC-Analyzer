...
const CPU_SCORES: Record<string, { score: number; cpu_mark?: number; single_thread?: number; tier?: string }> = {
  "Ryzen 7 H255": { score: 98, tier: "high" }, "Ryzen 7 8745HS": { score: 98, tier: "high" }, "Ryzen 7 8700G": { score: 96, tier: "high" },
  "Ryzen 7 7735HS": { score: 92, tier: "high" }, "Ryzen 9 6900HX": { score: 91, tier: "high" }, "Ryzen 7 6800U": { score: 88, tier: "high" },
  "Ryzen 7 5800H": { score: 84, tier: "high" }, "Ryzen 7 5800U": { score: 79, cpu_mark: 18800, single_thread: 3050, tier: "good" },
  "Ryzen 7 5700U": { score: 76, tier: "good" }, "Ryzen 7 5700X": { score: 86, cpu_mark: 26700, single_thread: 3350, tier: "high" }, "Ryzen 5 5650GE": { score: 72, tier: "good" }, "Ryzen 5 7430U": { score: 70, tier: "good" },
  "Ryzen 5 5500U": { score: 66, tier: "good" }, "Ryzen 5 unknown": { score: 55, tier: "estimated" }, "Ryzen 7 unknown": { score: 65, tier: "estimated" },
  "Ryzen 9 unknown": { score: 75, tier: "estimated" }, "Intel i5-12400": { score: 82, tier: "good" }, "Intel i5-1250P": { score: 78, tier: "good" },
  "Intel i5-1235U": { score: 74, tier: "good" }, "Intel i5-1135G7": { score: 62, tier: "office" }, "Intel i7-6700HQ": { score: 54, tier: "old" },
  "Intel i5-9500": { score: 52, tier: "old" }, "Intel i5-7500": { score: 45, tier: "old" }, "Intel i5-8500T": { score: 43, tier: "old_low_power" },
  "Intel i3-10th gen": { score: 40, tier: "estimated" }, "Intel i5-10th gen": { score: 48, tier: "estimated" }, "Intel i7-10th gen": { score: 56, tier: "estimated" },
  "Intel i5-6500T": { score: 35, tier: "old_low_power" }, "Intel i3-7th gen": { score: 24, tier: "estimated_old" }, "Intel N150": { score: 34, tier: "low_power" },
  "Intel N100": { score: 30, tier: "low_power" }, "Intel N4000": { score: 12, tier: "avoid" }, "Intel G630": { score: 8, tier: "avoid" }, "AMD A10": { score: 15, tier: "avoid" }
};
const GPU_SCORES: Record<string, { score: number; tier: string }> = {
  ...
  "RTX 3060": { score: 58, tier: "mid" }, "GTX 1660 SUPER": { score: 38, tier: "entry" }, "GTX 1660": { score: 34, tier: "entry" }, "GTX 1650": { score: 24, tier: "old" },
  "RX 7900 XTX": { score: 90, tier: "high" }, "RX 7800 XT": { score: 78, tier: "high" }, "RX 7700 XT": { score: 70, tier: "mid" }, "RX 7600": { score: 55, tier: "mid" },
  "RX 6800 XT": { score: 76, tier: "high" }, "RX 6750 XT": { score: 66, tier: "mid" }, "RX 6700 XT": { score: 62, tier: "mid" }, "RX 6600": { score: 42, tier: "entry" }, "Radeon 780M": { score: 26, tier: "integrated" },
...
function extractGpu(text: string): string | null { const upper = text.toUpperCase(); const patterns: Array<[RegExp, string]> = [[/\bRTX\s*(5090|5080|5070|4070|4060|3090|3080|3070|3060|2080|2070|2060)\b/i, "RTX {}"], [/\bGTX\s*(1660\s*SUPER|1660|1650|1050\s*TI)\b/i, "GTX {}"], [/\bRX\s*(7900\s*XTX|7800\s*XT|7700\s*XT|7600|6800\s*XT|6750\s*XT|6700\s*XT|6600)\b/i, "RX {}"], [/\bRADEON\s*(780M)\b/i, "Radeon {}"], [/\bARC\s*(A770|A750)\b/i, "Intel Arc {}"]]; for (const [regex, template] of patterns) { const match = upper.match(regex); if (match) return template.replace("{}", match[1].replace(/\s+/g, " ").trim().replace(/^(\d{4})(XT|XTX)$/, "$1 $2")); } return null; }
function extractRamGb(text: string): number | null { const values: number[] = []; for (const m of text.matchAll(/\b(\d{1,2})\s*x\s*(\d{1,2})\b/gi)) values.push(Number(m[1]) * Number(m[2])); for (const m of text.matchAll(/\b(\d{1,3})\s*(?:GO|GB)\b/gi)) { const gb = Number(m[1]); const before = text.slice(Math.max(0, m.index! - 24), m.index); const after = text.slice(m.index! + m[0].length, m.index! + m[0].length + 10); if (/\b(SSD|NVME|HDD|STOCKAGE|DISQUE|STORAGE|ROM|EMMC|SATA|M\.2|M2|DD)\s*(?::|de|d'|avec|en)?\s*$/i.test(before) || (gb >= 64 && /^\s*(SSD|NVME|HDD|ROM|EMMC|SATA|M\.2|M2|DD)\b/i.test(after))) continue; if (gb >= 2 && gb <= 128) values.push(gb); } return values.length ? Math.max(...values) : null; }

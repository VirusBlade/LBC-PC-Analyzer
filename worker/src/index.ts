...
function extractGpu(text: string): string | null { const upper = text.toUpperCase(); const patterns: Array<[RegExp, string]> = [[/\bRTX\s*(5090|5080|5070|4070|4060|3090|3080|3070|3060|2080|2070|2060)\b/i, "RTX {}"], [/\bGTX\s*(1660\s*SUPER|1660|1650|1050\s*TI)\b/i, "GTX {}"], [/\bRX\s*(7900\s*XTX|7800\s*XT|7700\s*XT|7600|6800\s*XT|6750\s*XT|6700\s*XT|6600|5700\s*XT)\b/i, "RX {}"], [/\bRADEON\s*(780M)\b/i, "Radeon {}"], [/\bARC\s*(A770|A750)\b/i, "Intel Arc {}"]]; for (const [regex, template] of patterns) { const match = upper.match(regex); if (match) return template.replace("{}", match[1].replace(/\s+/g, " ").trim().replace(/^(\d{4})(XT|XTX)$/, "$1 $2")); } return null; }
